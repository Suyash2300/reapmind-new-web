...
const url = "https://reapmind.com/top-react-native-development-company-in-mumbai/";
const publicDir = path.join(__dirname, "..", "public", "top-react-native-mumbai");
...
// Save text extract for reference
const textPath = path.join(publicDir, "page-text-extract.txt");
fs.writeFileSync(textPath, html.replace(/<script[\s\S]*?<\/script>/gi, "").replace(/<style[\s\S]*?<\/style>/gi, "").replace(/<[^>]+>/g, "\n").replace(/\n{3,}/g, "\n\n"));
