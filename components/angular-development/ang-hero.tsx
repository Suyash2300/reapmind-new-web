...
const COMPONENT_TREE = [
  { label: "AppModule", depth: 0 },
  { label: "DashboardComponent", depth: 1 },
  { label: "UserService", depth: 2 },
  { label: "DataBinding", depth: 2 },
] as const;

export function AngHero() {
  const { hero } = angularDevelopmentConfig;
  ...
  return (
    <section className="relative overflow-hidden bg-surface-dark pb-12 pt-10 text-primary-foreground md:pb-14 md:pt-12 lg:pt-14">
      <div className="pointer-events-none absolute inset-0" aria-hidden>
        ...
        />
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_85%_65%_at_50%_-15%,rgba(221,0,49,0.28),transparent)]" />
      ...
      <div className="container-app relative">
            ...
            {hero.breadcrumb.map((crumb, i) => (
              <li key={crumb.label} className="flex items-center gap-2">
                ...
                {i < hero.breadcrumb.length - 1 ? (
                  <Link href={crumb.href} className="transition-colors hover:text-[#DD0031]">
                    {crumb.label}
                  </Link>
                ) : (
        ...
        <div className="grid items-start gap-8 sm:gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)] lg:gap-12">
          <div>
            <div className="mt-1">
              <HeroWordHeading text={hero.heading} accentWord="Angular" />
            </div>
            ...
            >
              <Link
                ...
                className="inline-flex min-h-11 items-center justify-center rounded-full bg-[#DD0031] px-8 text-sm font-semibold text-white shadow-[0_12px_40px_-12px_rgba(221,0,49,0.55)] transition-transform hover:scale-[1.02] hover:bg-[#c4002b]"
              >
                {hero.cta}
              </Link>
            </motion.div>
            ...
            <div className="rounded-[1.75rem] border border-white/10 bg-black/55 p-5 backdrop-blur-md">
              <div className="flex items-center gap-3">
                <div className="relative h-12 w-12 shrink-0">
                  <Image src={hero.logo} alt="Angular" fill sizes="48px" className="object-contain" />
                </div>
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wider text-[#DD0031]">Component architecture</p>
                  <p className="text-sm font-bold text-white">MVC · DI · Two-way binding</p>
                </div>
              </div>

              <div className="mt-4 space-y-2 rounded-xl border border-white/10 bg-[#0a0608] p-3 font-mono text-[11px]">
                {COMPONENT_TREE.map((node, i) => (
                  ...
                  >
                    <span className="text-[#DD0031]" aria-hidden>
                      {node.depth === 0 ? "▸" : "└"}
                    </span>
              ...
              <div className="mt-3 grid grid-cols-3 gap-2 text-center">
                {[
                  { label: "Build", value: "AOT" },
                  { label: "Test", value: "Jasmine" },
                  ...
                  <motion.div
                    ...
                    className="rounded-lg border border-[#DD0031]/20 bg-[#DD0031]/10 px-2 py-2"
                  >
