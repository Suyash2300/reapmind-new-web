...
export function RpmHero() {
  const reduced = usePrefersReducedMotion();
  const chars = rpmConfig.hero.heading.split("");
      ...
      <div className="container-app relative z-10 grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
        <div>
          <nav className="mb-6 flex flex-wrap items-center gap-2 text-xs font-medium text-white/60 sm:text-sm">
            <Link href={rpmConfig.breadcrumb[0].href} className="hover:text-cyan-400">
              {rpmConfig.breadcrumb[0].label}
            </Link>
            <span>/</span>
            <span>{rpmConfig.breadcrumb[1].label}</span>
          </nav>
        ...
        >
          ...
          >
            <Image
              src={rpmConfig.hero.image}
              alt={rpmConfig.hero.heading}
              fill
              priority
