...
const ACCENTS = ["#1A69FD", "#10B981", "#F97316", "#8B5CF6"] as const;

export function B2mBusinessBenefits() {
  const { businessBenefits } = b2bMarketplaceConfig;
  const reducedMotion = usePrefersReducedMotion();

  return (
    <section className="border-t border-white/10 bg-black py-12 md:py-16 lg:py-20" aria-labelledby="b2m-benefits-heading">
      <div className="container-app">
        <BlurFadeIn as="h2" id="b2m-benefits-heading" className="max-w-3xl text-h3 font-bold text-white sm:text-h2">
          {businessBenefits.title}
        </BlurFadeIn>

        <StaggerGrid className="mt-10 grid gap-4 sm:grid-cols-2">
          {businessBenefits.items.map((item, i) => {
