...
const FLOW_STEPS = [
  { label: "Profile", sub: "Pet details" },
  { label: "Remind", sub: "Food & vet" },
  { label: "Track", sub: "GPS & health" },
  { label: "Care", sub: "Vet & first aid" },
] as const;

export function OpcHero() {
  const { hero } = onlinePetCareConfig;
  ...
  return (
    <section className="relative overflow-hidden bg-surface-dark pb-12 pt-10 text-primary-foreground md:pb-14 md:pt-12 lg:pt-14">
      <div className="pointer-events-none absolute inset-0" aria-hidden>
        ...
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_80%_60%_at_12%_0%,rgba(20,184,166,0.24),transparent_55%)]" />
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_55%_45%_at_88%_22%,rgba(26,105,253,0.14),transparent)]" />
      </div>
        ...
        <div className="grid items-start gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.95fr)] lg:gap-12">
          <div>
            <motion.p
              ...
              className="text-sm font-semibold uppercase tracking-[0.22em] text-teal-400"
            >
              {hero.badge}
            </motion.p>
            <div className="mt-3">
              <HeroWordHeading text={hero.heading} accentWord="Pet" />
            </div>
          ...
          >
            <div className="rounded-[1.75rem] border border-white/10 bg-black/45 p-5 backdrop-blur-md sm:p-6">
              <p className="text-xs font-semibold uppercase tracking-wider text-teal-400/90">Pet care flow</p>
              <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
                {FLOW_STEPS.map((step, i) => (
                  ...
                  >
                    <span className="text-[10px] font-bold tabular-nums text-teal-400/80">{String(i + 1).padStart(2, "0")}</span>
              ...
              {!reducedMotion && (
                <motion.div className="mt-4 h-1 overflow-hidden rounded-full bg-white/10" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }}>
                  <motion.div
                    className="h-full rounded-full bg-gradient-to-r from-teal-400 via-primary to-teal-400"
                    animate={{ x: ["-100%", "100%"] }}
