...
export function EtmsOfferings() {
  const { offerings } = employeeTransportationConfig;
  ...
  return (
    <section
      ...
      aria-labelledby="etms-offerings-heading"
    >
      <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(90deg,transparent,rgba(26,105,253,0.04),transparent)]" aria-hidden />

      <div className="container-app relative">
        <div className="grid gap-12 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.75fr)] lg:items-start lg:gap-16">
          <div>
            <BlurFadeIn as="h2" id="etms-offerings-heading" className="max-w-3xl text-h3 font-bold text-white sm:text-h2">
              {offerings.title}
            </BlurFadeIn>

            <div className="mt-8 space-y-6">
              {offerings.paragraphs.map((p, i) => (
          ...
          <div className="space-y-4">
            {offerings.stats.map((stat, i) => (
              ...
              >
                <div className="absolute inset-0 bg-gradient-to-r from-cyan-500/10 to-transparent opacity-0 transition-opacity group-hover:opacity-100" aria-hidden />
            ...
            >
              <Image
                ...
                alt="Corporate employee transport fleet"
                fill
                sizes="(max-width: 1024px) 100vw, 380px"
                className="object-cover object-center"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-primary/10" aria-hidden />
            </motion.div>
          </div>
        </div>
      </div>
