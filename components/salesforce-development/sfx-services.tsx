...
type ServiceId = (typeof salesforceDevelopmentConfig.services.items)[number]["id"];

export function SfxServices() {
  const { services } = salesforceDevelopmentConfig;
  const reducedMotion = usePrefersReducedMotion();
  const [activeId, setActiveId] = useState<ServiceId>("custom");
  const items = services.items;

  return (
    <section className="border-t border-white/10 bg-black py-12 md:py-16 lg:py-20" aria-labelledby="sfx-services-heading">
      <div className="container-app">
        <BlurFadeIn as="h2" id="sfx-services-heading" className="mx-auto max-w-4xl text-center text-h3 font-bold text-white sm:text-h2">
          {services.title}
        </BlurFadeIn>
        ...
        <div className="mt-10 flex flex-wrap justify-center gap-2" role="tablist" aria-label="Salesforce development services">
          {items.map((item) => {
