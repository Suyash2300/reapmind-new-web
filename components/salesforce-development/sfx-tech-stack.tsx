...
function StackTagIcon({ icon }: { icon: string }) {
  if (icon.startsWith("/")) {
    return (
...
export function SfxTechStack() {
  const { techStack } = salesforceDevelopmentConfig;
  const reducedMotion = usePrefersReducedMotion();

  return (
    <section className="border-t border-white/10 bg-black py-12 md:py-16 lg:py-20" aria-labelledby="sfx-tech-heading">
      <div className="container-app">
        <BlurFadeIn as="h2" id="sfx-tech-heading" className="max-w-4xl text-h3 font-bold text-white sm:text-h2">
          {techStack.title}
        </BlurFadeIn>
