...
import { isNextjsIcon, NextjsLogoMark } from "@/components/nextjs-development/nextjs-logo-mark";

const smoothEase = [0.22, 1, 0.36, 1] as const;

function StackTagIcon({ icon, name }: { icon: string; name?: string }) {
  if (isNextjsIcon(icon, name)) {
    return <NextjsLogoMark size={16} />;
  }
...
export function NdxTechStack() {
  const { techStack } = nextjsDevelopmentConfig;
  const reducedMotion = usePrefersReducedMotion();

  return (
    <section className="border-t border-white/10 bg-black py-12 md:py-16 lg:py-20" aria-labelledby="ndx-tech-heading">
      <div className="container-app">
        <BlurFadeIn as="h2" id="ndx-tech-heading" className="max-w-4xl text-h3 font-bold text-white sm:text-h2">
          {techStack.title}
        </BlurFadeIn>
