...
import { imageVideoSharingInsights } from "@/lib/image-video-sharing-config";

export function IvsInsightsSection() {
  const reducedMotion = usePrefersReducedMotion();

  return (
    <section className="border-t border-white/10 bg-surface-dark py-12 md:py-16" aria-labelledby="ivs-insights-heading">
      <div className="container-app">
        <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
          <BlurFadeIn as="h2" id="ivs-insights-heading" className="text-h3 font-bold text-white sm:text-h2">
            Latest Insights
          </BlurFadeIn>
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {imageVideoSharingInsights.map((article, index) => (
            <motion.div
              key={`${article.link}-${article.date}`}
              initial={reducedMotion ? false : { opacity: 0, y: 50, rotateX: 8 }}
            ...
            >
              <Link
                href={article.link}
                className="group block h-full overflow-hidden rounded-2xl border border-white/10 bg-black/40"
              >
