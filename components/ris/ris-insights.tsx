...
export function RisInsights() {
  const reduced = usePrefersReducedMotion();
  const ref = useRef<HTMLElement>(null);
  const { scrollYProgress } = useScroll({ target: ref, offset: ["start end", "end start"] });
  const headerY = useTransform(scrollYProgress, [0, 1], [20, -20]);
  const articles = risConfig.insights.articles;

  return (
    <section ref={ref} className="section-app border-t border-white/10 bg-surface-dark py-20 sm:py-24">
      <div className="container-app">
        <motion.div
          className="mb-10 flex flex-wrap items-end justify-between gap-4"
          style={reduced ? undefined : { y: headerY }}
        >
          <h2 className="text-h2 font-black text-white md:text-display">{risConfig.insights.title}</h2>
          <Link
            href="/blogs"
        ...
        <div className="columns-1 gap-5 sm:columns-2 lg:columns-3">
          {articles.map((article, index) => (
            <motion.div
              ...
              initial={reduced ? false : { opacity: 0, y: 30, rotate: index % 2 === 0 ? -1 : 1 }}
            ...
            >
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <h3 className="mt-2 line-clamp-3 text-base font-bold text-white group-hover:text-violet-300">
                    {article.title}
                  </h3>
