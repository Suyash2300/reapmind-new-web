...
export function RisTestimonials() {
  const reduced = usePrefersReducedMotion();
  const [active, setActive] = useState(0);
  const items = risConfig.testimonials.items;
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-12">
          ...
          <div className="relative min-h-[220px] overflow-hidden rounded-3xl border border-white/10 bg-surface-elevated p-6 sm:p-8 lg:col-span-8">
            <AnimatePresence mode="wait">
              <motion.div
                key={item.name}
                initial={reduced ? false : { opacity: 0, rotateY: 25, z: -50 }}
                animate={reduced ? undefined : { opacity: 1, rotateY: 0, z: 0 }}
                exit={reduced ? undefined : { opacity: 0, rotateY: -25, z: -50 }}
                transition={{ duration: 0.4 }}
                style={{ transformStyle: "preserve-3d" }}
              >
                <p className="text-xs font-semibold uppercase tracking-wider text-violet-400">{item.role}</p>
