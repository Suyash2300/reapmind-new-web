...
export function TbsWhyReapmind() {
  const { whyUs } = transportBookingConfig;
  const reducedMotion = usePrefersReducedMotion();
  const [innovation, commitment, quality] = whyUs.items;

  return (
    <section className="border-t border-white/10 bg-surface-dark py-12 md:py-16 lg:py-20" aria-labelledby="tbs-why-heading">
      <div className="container-app">
        <BlurFadeIn as="h2" id="tbs-why-heading" className="max-w-4xl text-h3 font-bold text-white sm:text-h2">
          {whyUs.title}
        </BlurFadeIn>
