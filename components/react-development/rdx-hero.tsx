...
const COMPONENT_TREE = [
  { label: "App", depth: 0 },
  { label: "Dashboard", depth: 1 },
  { label: "useState()", depth: 2 },
  { label: "useEffect()", depth: 2 },
] as const;

export function RdxHero() {
  const { hero } = reactDevelopmentConfig;
    ...
    <section className="relative overflow-hidden bg-surface-dark pb-12 pt-10 text-primary-foreground md:pb-14 md:pt-12 lg:pt-14">
      <div className="pointer-events-none absolute inset-0" aria-hidden>
        ...
        <div
          ...
          style={{ background: `radial-gradient(ellipse 85% 65% at 50% -15%, rgba(97,218,251,0.28), transparent)` }}
        />
      ...
      <div className="container-app relative">
            ...
            {hero.breadcrumb.map((crumb, i) => (
              <li key={crumb.label} className="flex items-center gap-2">
                ...
                {i < hero.breadcrumb.length - 1 ? (
                  <Link href={crumb.href} className="transition-colors hover:text-[#61DAFB]">
                    {crumb.label}
                  </Link>
                ) : (
        ...
        <div className="grid items-start gap-8 sm:gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)] lg:gap-12">
          <div>
            <div className="mt-1">
              <HeroWordHeading text={hero.heading} accentWord="React.js" />
            </div>
            ...
            >
              <Link
                ...
                className="inline-flex min-h-11 items-center justify-center rounded-full px-8 text-sm font-semibold text-[#0a1628] shadow-[0_12px_40px_-12px_rgba(97,218,251,0.55)] transition-transform hover:scale-[1.02]"
            ...
            <div className="rounded-[1.75rem] border border-white/10 bg-black/55 p-5 backdrop-blur-md">
              <div className="flex items-center gap-3">
                <div className="relative h-12 w-12 shrink-0">
                  <Image src={hero.logo} alt="React" fill sizes="48px" className="object-contain" />
                </div>
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: REACT_ACCENT }}>
                    Component model
                  </p>
                  <p className="text-sm font-bold text-white">Hooks · JSX · Virtual DOM</p>
                </div>
              </div>

              <div className="mt-4 space-y-2 rounded-xl border border-white/10 bg-[#0a1218] p-3 font-mono text-[11px]">
              ...
              <div className="mt-3 grid grid-cols-3 gap-2 text-center">
                {[
                  ...
                  <motion.div
                    ...
                    style={{ borderColor: `${REACT_ACCENT}33`, backgroundColor: `${REACT_ACCENT}18` }}
                  >
