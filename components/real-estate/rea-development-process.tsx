...
export function ReaDevelopmentProcess() {
  const { developmentProcess } = realEstateConfig;
  const reducedMotion = usePrefersReducedMotion();
  const [active, setActive] = useState(0);
  const steps = developmentProcess.steps;

  return (
    <section className="border-t border-white/10 bg-surface-dark py-12 md:py-16 lg:py-20" aria-labelledby="rea-process-heading">
      <div className="container-app">
        <BlurFadeIn as="h2" id="rea-process-heading" className="max-w-4xl text-h3 font-bold text-white sm:text-h2">
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-[minmax(0,0.42fr)_minmax(0,1fr)] lg:items-start">
          <div className="relative pl-4 sm:pl-6">
            <div className="absolute bottom-2 left-[1.35rem] top-2 w-px bg-white/10 sm:left-[1.6rem]" aria-hidden>
              <motion.div
                className="w-full origin-top bg-amber-400"
                animate={{ height: `${((active + 1) / steps.length) * 100}%` }}
                transition={{ duration: reducedMotion ? 0 : 0.35, ease: smoothEase }}
              />
            </div>

            <div className="space-y-1.5" role="tablist" aria-label="Real estate development process">
              {steps.map((step, i) => {
                ...
                return (
                  <HydrationButton
                    ...
                    onClick={() => setActive(i)}
                    className={`relative flex w-full items-center gap-3 rounded-xl border py-2.5 pl-10 pr-3 text-left transition-colors sm:py-3 sm:pl-12 ${
                      isActive ? "border-white/20 bg-black/60" : "border-transparent hover:bg-black/30"
                    }`}
                  >
                    <span
                      className={`absolute left-0 flex h-7 w-7 items-center justify-center rounded-full border text-xs sm:h-8 sm:w-8 ${
                        isActive
                    ...
                    <span className={`text-xs font-semibold sm:text-sm ${isActive ? "text-white" : "text-white/55"}`}>
                      {step.title}
                    </span>
                  </HydrationButton>
                );
          ...
          <div className="relative min-h-[260px] sm:min-h-[280px]" role="tabpanel">
            {steps.map((step, i) => {
              const isActive = i === active;
              return (
                <motion.article
