...
export function AibSectorCards() {
  const { aiSectors } = aiBusinessConfig;
  const reducedMotion = usePrefersReducedMotion();
  const [active, setActive] = useState(0);
  const [imgFailed, setImgFailed] = useState<Record<string, boolean>>({});
  const sector = aiSectors.items[active];
  const sectorImage =
  ...
  return (
    <section className="border-t border-white/10 bg-surface-dark py-12 md:py-16 lg:py-20" aria-labelledby="aib-sectors-heading">
      <div className="container-app">
        <BlurFadeIn as="h2" id="aib-sectors-heading" className="text-h3 font-bold text-white sm:text-h2">
          {aiSectors.title}
        </BlurFadeIn>

        <div className="mt-10 grid gap-6 lg:grid-cols-[minmax(0,0.38fr)_minmax(0,1fr)] lg:items-stretch lg:gap-8">
          <div className="flex flex-wrap gap-2 lg:flex-col" role="tablist" aria-label="AI industry sectors">
            {aiSectors.items.map((s, i) => (
              <HydrationButton
                key={s.id}
                type="button"
                role="tab"
                ...
                style={
                  i === active
                    ? { borderColor: `${s.accent}66`, backgroundColor: `${s.accent}14` }
                    : undefined
                }
              >
                {s.title}
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="p-5 sm:p-7">
                <p className="text-para leading-relaxed text-white/65">{sector.body}</p>
                <Link
