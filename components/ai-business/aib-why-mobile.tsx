...
function PillarCard({
  ...
  return (
    ...
    >
      <h3 className="flex items-center gap-2 text-subtitle font-bold text-white">
        <span
...
export function AibWhyMobile() {
  const { whyMobile } = aiBusinessConfig;
  const reducedMotion = usePrefersReducedMotion();
  const [innovation, commitment, quality] = whyMobile.items;

  return (
    <section className="border-t border-white/10 bg-black py-12 md:py-16 lg:py-20" aria-labelledby="aib-why-mobile-heading">
      <div className="container-app">
        <BlurFadeIn as="h2" id="aib-why-mobile-heading" className="max-w-4xl text-h3 font-bold text-white sm:text-h2">
          {whyMobile.title}
        </BlurFadeIn>
        <BlurFadeIn as="p" delay={0.06} className="mt-4 max-w-4xl text-para leading-relaxed text-white/65">
          <WordReveal text={whyMobile.intro} delay={0.08} />
        </BlurFadeIn>
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] lg:grid-rows-3 lg:items-stretch lg:gap-6">
          ...
          <FmTestimonialVideo
            src={whyMobile.video.src}
            poster={whyMobile.video.poster}
            title={whyMobile.video.title}
            fillHeight
