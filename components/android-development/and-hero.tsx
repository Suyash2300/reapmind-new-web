...
export function AndHero() {
  const { hero } = androidDevelopmentConfig;
  ...
  return (
    <section className="relative overflow-hidden bg-surface-dark pb-12 pt-10 text-primary-foreground md:pb-14 md:pt-12 lg:pt-14">
      <div className="pointer-events-none absolute inset-0" aria-hidden>
        <Image
          src={bgFailed ? "/android-hire/hero.jpg" : hero.image}
          alt=""
          fill
          sizes="100vw"
        ...
        <div
          ...
          style={{ background: `radial-gradient(ellipse 85% 65% at 50% -15%, rgba(61,220,132,0.28), transparent)` }}
        />
      ...
      <div className="container-app relative">
            ...
            {hero.breadcrumb.map((crumb, i) => (
              <li key={crumb.label} className="flex items-center gap-2">
                ...
                {i < hero.breadcrumb.length - 1 ? (
                  <Link href={crumb.href} className="transition-colors hover:text-[#3DDC84]">
                    {crumb.label}
                  </Link>
                ) : (
        ...
        <div className="grid items-start gap-8 sm:gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.95fr)] lg:gap-12">
          <div>
            <MountBlurFade as="p" className="text-sm font-semibold uppercase tracking-[0.22em] text-[#3DDC84]">
              {hero.badge}
            </MountBlurFade>

            <div className="mt-3">
              <HeroWordHeading text={hero.heading} accentWord="Android" />
            </div>
            ...
            >
              <Link
                ...
                className="inline-flex min-h-11 items-center justify-center rounded-full px-8 text-sm font-semibold text-black shadow-[0_12px_40px_-12px_rgba(61,220,132,0.55)] transition-transform hover:scale-[1.02]"
