      ...
      <div className="container-app relative z-10">
        <GsapScrollReveal className="text-center max-w-4xl mx-auto mb-20">
          <h2 className="text-display font-black text-white leading-tight mb-6">
            {mumbaiModConfig.benefits.title}
          </h2>
          <p className="text-h6 text-white/70 leading-relaxed font-normal">
            {mumbaiModConfig.benefits.subtitle}
          </p>
        </GsapScrollReveal>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {mumbaiModConfig.benefits.items.map((item, i) => (
            <GsapScrollReveal key={item.title} start={`top ${80 + (i % 3) * 10}%`}>
              <motion.div 
                className="group h-full bg-white/5 border border-white/10 rounded-[2rem] p-10 hover:bg-surface-dark transition-colors duration-500"
                whileHover={{ y: -5 }}
              >
