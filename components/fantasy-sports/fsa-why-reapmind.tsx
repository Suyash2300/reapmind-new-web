...
export function FsaWhyReapmind() {
  const { whyUs } = fantasySportsConfig;
  const reducedMotion = usePrefersReducedMotion();
  const [expertise, innovation, advantage] = whyUs.items;

  return (
    <section className="border-t border-white/10 bg-black py-12 md:py-16 lg:py-20" aria-labelledby="fsa-why-heading">
      <div className="container-app">
        <BlurFadeIn as="h2" id="fsa-why-heading" className="max-w-4xl text-h3 font-bold text-white sm:text-h2">
          {whyUs.title}
        </BlurFadeIn>
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] lg:grid-rows-3 lg:items-stretch lg:gap-6">
          <BlurFadeIn delay={0.04} className="h-full lg:col-start-1 lg:row-start-1">
            <PillarCard {...expertise} reducedMotion={reducedMotion} />
          </BlurFadeIn>
          ...
          <BlurFadeIn delay={0.12} className="h-full lg:col-start-1 lg:row-start-3">
            <PillarCard {...advantage} reducedMotion={reducedMotion} />
          </BlurFadeIn>
        </div>
