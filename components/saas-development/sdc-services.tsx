...
type ServiceId = (typeof saasDevelopmentConfig.services.items)[number]["id"];

export function SdcServices() {
  const { services } = saasDevelopmentConfig;
  const reducedMotion = usePrefersReducedMotion();
  const [activeId, setActiveId] = useState<ServiceId>("consulting");
  const items = services.items;

  return (
    <section className="border-t border-white/10 bg-surface-dark py-12 md:py-16 lg:py-20" aria-labelledby="sdc-services-heading">
      <div className="container-app">
        <BlurFadeIn as="h2" id="sdc-services-heading" className="mx-auto max-w-4xl text-center text-h3 font-bold text-white sm:text-h2">
          {services.title}
        </BlurFadeIn>

        <div className="mt-10 flex flex-wrap justify-center gap-2 sm:gap-3" role="tablist" aria-label="SaaS development services">
          {items.map((item) => {
        ...
        <div className="relative mx-auto mt-8 max-w-4xl min-h-[300px] sm:min-h-[340px]" role="tabpanel">
          {items.map((item, i) => {
