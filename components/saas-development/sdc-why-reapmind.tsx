...
export function SdcWhyReapmind() {
  const { whyUs } = saasDevelopmentConfig;
  const reducedMotion = usePrefersReducedMotion();
  const [innovation, satisfaction] = whyUs.items;

  return (
    <section className="border-t border-white/10 bg-surface-dark py-12 md:py-16 lg:py-20" aria-labelledby="sdc-why-heading">
      <div className="container-app">
        <BlurFadeIn as="h2" id="sdc-why-heading" className="max-w-4xl text-h3 font-bold text-white sm:text-h2">
          {whyUs.title}
        </BlurFadeIn>
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] lg:grid-rows-2 lg:items-stretch lg:gap-6">
          ...
          <FmTestimonialVideo
            ...
            fillHeight
            className="h-full min-h-[240px] lg:col-start-2 lg:row-start-1 lg:row-span-2 lg:min-h-0"
          />

          <BlurFadeIn delay={0.08} className="h-full lg:col-start-1 lg:row-start-2">
            <PillarCard {...satisfaction} reducedMotion={reducedMotion} />
          </BlurFadeIn>
        </div>
