...
export function IdxCustomProcess() {
  const { customProcess } = ionicDevelopmentConfig;
  const reducedMotion = usePrefersReducedMotion();

  return (
    <section className="border-t border-white/10 bg-black py-12 md:py-16 lg:py-20" aria-labelledby="idx-process-heading">
      <div className="container-app">
        <BlurFadeIn as="h2" id="idx-process-heading" className="max-w-4xl text-h3 font-bold text-white sm:text-h2">
          {customProcess.title}
        </BlurFadeIn>
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ delay: 0.12, duration: 0.55, ease: smoothEase }}
          className="mt-12 rounded-[1.75rem] border p-6 sm:p-8"
          style={{
            borderColor: `${IONIC_ACCENT}33`,
            background: `linear-gradient(to bottom right, ${IONIC_ACCENT}14, black, rgba(97,218,251,0.05))`,
          }}
        >
