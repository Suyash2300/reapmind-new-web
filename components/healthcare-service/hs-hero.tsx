...
export function HsHero() {
  ...
  const chars = config.hero.heading.split("");
      ...
      <div className="container-app relative z-10 grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
        <div>
          <nav className="mb-6 flex flex-wrap items-center gap-2 text-xs font-medium text-white/60 sm:text-sm">
            <Link href={config.breadcrumb[0].href} className="hover:text-cyan-400">
              {config.breadcrumb[0].label}
            </Link>
            <span>/</span>
            <span>{config.breadcrumb[1].label}</span>
          </nav>
          ...
          <div className="mt-7 space-y-4">
            {(config.hero.paragraphs?.length
              ? config.hero.paragraphs
              : [config.hero.description]
            ).map((paragraph) => (
              <motion.p
                key={paragraph.slice(0, 48)}
                className="text-para leading-relaxed text-white/72"
