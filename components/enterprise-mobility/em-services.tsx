        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {enterpriseMobilityConfig.services.items.map((service, i) => (
            <GsapScrollReveal key={service.title} start={`top ${85 + (i % 3) * 5}%`}>
              <motion.div
                className="group relative flex flex-col h-full overflow-hidden rounded-3xl border border-white/5 bg-white/5 p-8 transition-all duration-500 hover:bg-white/10 hover:border-primary/30"
                whileHover={{ y: -8 }}
              >
                {/* Number Watermark */}
