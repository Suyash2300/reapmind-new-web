...
export function MbaChallenges() {
  const { challenges } = mobileBankingConfig;
  const reducedMotion = usePrefersReducedMotion();

  return (
    <section className="border-t border-white/10 bg-surface-dark py-12 md:py-16 lg:py-20" aria-labelledby="mba-challenges-heading">
      <div className="container-app">
        <BlurFadeIn as="h2" id="mba-challenges-heading" className="max-w-4xl text-h3 font-bold text-white sm:text-h2">
          {challenges.title}
        </BlurFadeIn>
        <BlurFadeIn as="p" delay={0.06} className="mt-4 max-w-4xl text-para leading-relaxed text-white/65">
          <WordReveal text={challenges.intro} delay={0.08} />
        </BlurFadeIn>
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {challenges.items.map((item, i) => (
            ...
            >
              ...
              <div className="mt-4 h-1 w-10 rounded-full" style={{ backgroundColor: item.accent }} aria-hidden />
            </motion.article>
          ))}
        </div>
      </div>
