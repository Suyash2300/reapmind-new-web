...
export function MbaPartnerIntro() {
  const { partnerIntro } = mobileBankingConfig;
  const reducedMotion = usePrefersReducedMotion();
  const [imgFailed, setImgFailed] = useState(false);
  const visualSrc = imgFailed ? partnerIntro.visual.fallback : partnerIntro.visual.src;

  return (
    <section
      className="relative overflow-hidden border-t border-white/10 bg-black py-12 text-primary-foreground md:py-16 lg:py-20"
      aria-labelledby="mba-partner-heading"
    >
      <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(135deg,rgba(26,105,253,0.05),transparent_50%,rgba(16,185,129,0.04))]" aria-hidden />

      <div className="container-app relative">
        <div className="grid gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)] lg:items-center lg:gap-14">
          <div>
            <BlurFadeIn as="h2" id="mba-partner-heading" className="max-w-3xl text-h3 font-bold text-white sm:text-h2">
              {partnerIntro.title}
            </BlurFadeIn>

            {partnerIntro.paragraphs.map((p, i) => (
          ...
          <motion.div
            ...
            className="relative"
          >
            <div className="relative aspect-[4/5] overflow-hidden rounded-[1.75rem] border border-white/10 bg-black shadow-[0_32px_80px_-30px_rgba(26,105,253,0.35)] sm:aspect-[3/4]">
              <Image
                src={visualSrc}
                alt={partnerIntro.visual.alt}
                fill
                sizes="(max-width: 1024px) 100vw, 420px"
                className="object-contain object-center p-6 sm:p-10"
                onError={() => setImgFailed(true)}
              />
              <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(26,105,253,0.1),transparent_72%)]" aria-hidden />
            </div>
            ...
            >
              <p className="text-[10px] font-bold uppercase tracking-wider text-emerald-400">FinTech ready</p>
              <p className="mt-1 text-sm font-semibold text-white">Core banking · UPI · Biometrics</p>
            </motion.div>
          </motion.div>
        </div>
      </div>
