...
function PillarCard({
  ...
  const y = useTransform(scrollYProgress, [0, 1], [index % 2 === 0 ? 24 : -24, index % 2 === 0 ? -12 : 12]);
  ...
  return (
    <motion.article
      ...
      transition={{ delay: index * 0.1, duration: 0.65, ease: smoothEase }}
      whileHover={reducedMotion ? undefined : { borderColor: "rgba(168,85,247,0.4)" }}
...
export function SnWhyBuild() {
  const { whyBuild } = socialNetworkingConfig;
  ...
  return (
    <section className="border-t border-white/10 bg-surface-dark py-12 md:py-16 lg:py-20" aria-labelledby="sn-why-heading">
      <div className="container-app">
        <BlurFadeIn as="h2" id="sn-why-heading" className="max-w-4xl text-h3 font-bold text-white sm:text-h2">
          {whyBuild.title}
        </BlurFadeIn>
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] lg:grid-rows-3 lg:items-stretch lg:gap-6">
          ...
          <motion.div ref={videoRef} style={reducedMotion ? undefined : { y: videoY }} className="h-full lg:col-start-2 lg:row-start-1 lg:row-span-3">
            <FmTestimonialVideo
