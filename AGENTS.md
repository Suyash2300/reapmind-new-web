...
- **Unique sections (required):** every new homepage/marketing block needs a **distinct concept** — not a generic card grid, basic carousel, or copy of an existing section. Lead with one memorable idea (metaphor, layout, or interaction) that fits ReapMind’s dark premium tone. Reference: `HomeBrilliantProfessionals` (polaroid desk + dossier). Interaction must be **reliable** (no fragile shared-layout tricks that break clicks); polish with motion only after the core UX works. Mobile gets its own simplified variant of the same concept, not a stripped desktop afterthought.
